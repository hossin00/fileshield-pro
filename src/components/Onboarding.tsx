...
const SLIDES = [{"accent": "#3b82f6", "icon": "\ud83d\udee1\ufe0f", "title": "Encrypt any file\nin seconds", "desc": "Drag any file into FileShield and protect it with AES-256 encryption. Zero cloud, zero compromise."}, {"accent": "#3b82f6", "icon": "\ud83d\udcc1", "title": "Your personal\nencrypted vault", "desc": "Organize encrypted files in secure folders. Everything stays on your device, protected by your password."}, {"accent": "#3b82f6", "icon": "\ud83d\udd11", "title": "One password.\nTotal privacy.", "desc": "One master password protects all your files. Without it, no one can access your data \u2014 ever."}];
export function Onboarding({ onDone }: { onDone: () => void }) {
  const [idx, setIdx] = useState(0);
  const slide = SLIDES[idx];
  return (
    <div style={{ minHeight:'100vh', display:'flex', flexDirection:'column', background:'#080e1f', overflow:'hidden', position:'relative' }}>
      ...
      <div style={{ padding:'20px 24px', display:'flex', justifyContent:'flex-end' }}>
        <button onClick={onDone} style={{ color:'#3b82f660', fontSize:'14px', background:'none', border:'none', cursor:'pointer', fontFamily:'Inter, sans-serif' }}>Skip</button>
      </div>
      <div key={idx} style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center',
        ...
        <p style={{ color:'#3b82f680', fontSize:'15px', lineHeight:'1.7', maxWidth:'280px' }}>{slide.desc}</p>
      </div>
